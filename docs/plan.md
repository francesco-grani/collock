...
**How**: Add a one-off button in the sidebar ("Critique this app") that sends the current `build_system_prompt()` output to the LLM with a meta-prompt asking for usability, security, and prompt quality feedback. Display the result in an expander.
